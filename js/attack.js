...
function getResults() {
  getResponse("attackInfo.json", function(responseText) {
    ...
    var aps = "";
    var clients = "";
    var tr = "<thead><tr><th>Attack type</th><th>Status</th><th>Start/Stop</th></tr></thead>";
    ...
    for (var i = 0; i < res.attacks.length; i++) {
      ...
      if (res.attacks[i].status == "ready") tr += "<td style='color:#4CAF50;font-weight:bold;'>" + res.attacks[i].status + "</td>";
      else tr += "<td style='color:#f44336;font-weight:bold;'>" + res.attacks[i].status + "</td>";
      if (res.attacks[i].running) tr += "<td><button class='mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-color--red mdl-color-text--white' onclick='startStop(" + i + ")'>stop</button></td>";
      else tr += "<td><button class='mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-color--green mdl-color-text--white' onclick='startStop(" + i + ")'>start</button></td>";
